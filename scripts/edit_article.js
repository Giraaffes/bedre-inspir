// This strange function originates from the page's inline scripts
function addMagazineInput(type) {
	let newInput = '';
	if (type == 'illustration') {
		newInput = '<div class="form-data" id="formIndex100">' +
			'<span class="content-span" title="Ryk op" id="upArrow"> <img src="/images/app/icon/arrow-up-solid.svg"> </span> <span class="content-span" title="Ryk ned" id="downArrow"> <img src="/images/app/icon/arrow-down-solid.svg"> </span> <span class="content-span" title="Slet" id="deleteContent"><img src="/images/app/icon/x-solid.svg"></span>' +
			'<h5 class="input-label">Illustration</h5>' +
			'<div class="article-input-style" id="cropImagePreview">' +
			'<img id="cropPreview" name="content[' + type + '100]" src="/images/no_image_available.png" width="100%">' +
			'<input class="uploadCropImage file-input article-input-style" name="content[' + type + '100]" type="file" accept="image/*" placeholder="Choose File">' +
			'</div>' +
			'<input type="text" class="article-input-style" name="content[' + type + '100][caption]" placeholder="Billedtekst" value="">' +
			'<input id="articleImage" type="hidden" name="content[' + type + '100][value]">';
	} else {
		let rows = '';
		let inputLabel = '';
		let captionInput = '';
		...
		switch (type) {
			...
			case 'middle-heading':
				rows = '2';
				inputLabel = 'Mellemrubrik';
				break;
			case 'question':
				rows = '2';
				inputLabel = 'Spørgsmål';
				captionInput = '<input type="text" class="article-input-style" name="content[' + type + '100][caption]" placeholder="Navn/titel på den, der spørger" value="">';
				captionValue = '[value]';
				break;
			case 'answer':
				rows = '4';
				inputLabel = 'Svar';
				captionInput = '<input type="text" class="article-input-style" name="content[' + type + '100][caption]" placeholder="Navn på den, der svarer" value="">';
				captionValue = '[value]';
				break;
			case 'link':
				rows = '4';
				inputLabel = 'Afsluttende link';
				captionInput = '<input type="text" class="article-input-style" name="content[' + type + '100][caption]" placeholder="Afsluttende link" value="">';
				captionValue = '[value]';
				break;
			case 'socials':
				rows = '4';
				inputLabel = 'SoMe';
				extraClass = 'socialsInput'
				break;
		}

		newInput = '<div class="form-data" id="formIndex100">';
		newInput += '<span class="content-span" title="Ryk op" id="upArrow"> <img src="/images/app/icon/arrow-up-solid.svg"> </span>'
			+ '<span class="content-span" title="Ryk ned" id="downArrow"> <img src="/images/app/icon/arrow-down-solid.svg"> </span>'
			+ ' <span class="content-span" title="Slet" id="deleteContent"><img src="/images/app/icon/x-solid.svg"></span>';
		newInput += captionInput;
		newInput += '<h5 class="input-label">' + inputLabel + '</h5>';
		newInput += '<textarea rows="' + rows + '" class="article-input-style ' + extraClass + '" name="content[' + type + '100]' + captionValue + '" placeholder=""></textarea>';
	}
	newInput += '</div>';

	$("#form-inputs").append(newInput);
}
			...
			div.append(addButton(text, async () => {
					...
					if (name == "illustration") {
							...
							if (fileInput[0].files.length == 0) {
								...
								disableSaving = true; // Because page will save and reload automatically
							}
							return;
					}
...
label .fas { width: 16px; text-align: center; }
	...
	// Extra
	$("<p>(Nogle gange er billeder langsomme om at opdatere - genstart evt. din browser)</p>").css({
			"color": "#555555"
	}).appendTo("#magazines-articles-form > div:last > .form-data:eq(3)");

	let thumbnailFileInput = $(".file-input[name=\"magazinesArticlesImage\"]");
	thumbnailFileInput.on("change", () => {
		if (thumbnailFileInput[0].files.length > 0) disableSaving = true;  // Because page will save and reload automatically
	});
});
